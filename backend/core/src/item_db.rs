...
/// `<simc class>/<simc spec>` -> SimC consumable key -> value, from
/// `season-consumables.json` (generated by `fetch-season-consumables.mjs`).
static SEASON_CONSUMABLES: OnceCell<HashMap<String, HashMap<String, String>>> = OnceCell::new();
...
/// Game names Raidbots does not localize, per locale, from
/// `localized-names.<locale>.json` (generated by `fetch-localized-names.mjs`).
static LOCALIZED_NAMES: OnceCell<HashMap<String, LocalizedNames>> = OnceCell::new();
...
pub struct LocalizedNames {
    ...
    pub instances: HashMap<u64, String>,
    pub encounters: HashMap<u64, String>,
    pub currencies: HashMap<u64, String>,
    #[serde(rename = "nameDescriptions")]
    pub name_descriptions: HashMap<u64, String>,
    pub maps: HashMap<u64, String>,
    pub classes: HashMap<u64, String>,
    pub specs: HashMap<u64, String>,
    pub difficulties: HashMap<u64, String>,
}
...
pub fn load(data_dir: &Path) -> Result<(), String> {
    ...
    // season-consumables.json — SimulationCraft's own per-spec consumables, used
    // when the request names none. Absent until fetch-season-consumables.mjs has
    // run; missing is non-fatal, sims just carry no consumables (as before).
    let consumables_path = data_dir.join("season-consumables.json");
    if consumables_path.exists() {
        let file = fs::File::open(&consumables_path)
            .map_err(|e| format!("open {}: {}", consumables_path.display(), e))?;
        let raw: Value = serde_json::from_reader(std::io::BufReader::new(file))
            .map_err(|e| format!("parse {}: {}", consumables_path.display(), e))?;
        let specs: HashMap<String, HashMap<String, String>> = raw
            .get("specs")
            ...
            .map(|specs| {
                specs
                    ...
                    .filter_map(|(key, entry)| {
                        let values: HashMap<String, String> = entry
                            .as_object()?
                            .iter()
            ...
            .unwrap_or_default();
        println!("Loaded recommended consumables for {} specs", specs.len());
        let _ = SEASON_CONSUMABLES.set(specs);
    }
    ...
    let mut item_names: HashMap<u64, HashMap<String, String>> = HashMap::new();
    let mut have_item_names = false;
                ...
                if let Ok(id) = id_str.parse::<u64>() {
                    if let Some(obj) = locales.as_object() {
                        ...
                        if !locale_map.is_empty() {
                            item_names.insert(id, locale_map);
                        }
                    }
                }
            }
        }
        println!("Loaded {} localized item names", item_names.len());
        have_item_names = true;
    }

    // localized-names.<locale>.json — wago.tools bundles for every name
    // Raidbots does not localize. Their `items` merge into ITEM_NAMES (the file
    // wins for that locale, it is the newer client export); the smaller tables
    // are served as a bundle and read by the on-demand localize endpoint.
    let mut localized: HashMap<String, LocalizedNames> = HashMap::new();
    // An unlistable data dir is not a new error class: every file above is
    // already loaded with an `exists()` guard, so an absent dir means "no data".
    let mut localized_paths: Vec<std::path::PathBuf> = fs::read_dir(data_dir)
        .map(|entries| {
            entries
                .flatten()
                .map(|e| e.path())
                .filter(|p| localized_names_locale(p).is_some())
                .collect()
        })
        .unwrap_or_default();
    localized_paths.sort();
    for path in localized_paths {
        ...
        merge_item_names(&mut item_names, &locale, items);
        have_item_names = true;
        localized.insert(locale, names);
    }
    let _ = LOCALIZED_NAMES.set(localized);
    if have_item_names {
        let _ = ITEM_NAMES.set(item_names);
    }
...
/// The locale of a `localized-names.<locale>.json` path, or `None` for any
/// other file in the data dir.
fn localized_names_locale(path: &Path) -> Option<String> {
    let name = path.file_name()?.to_str()?;
    let locale = name
        .strip_prefix("localized-names.")?
        .strip_suffix(".json")?;
    // `xx_XX` only — `localized-names.json` or a stray `.bak` is not a locale.
    let (lang, region) = locale.split_once('_')?;
    if lang.len() == 2 && region.len() == 2 && locale.is_ascii() {
...
fn merge_item_names(
    base: &mut HashMap<u64, HashMap<String, String>>,
    locale: &str,
    items: HashMap<u64, String>,
) {
    for (id, name) in items {
        base.entry(id).or_default().insert(locale.to_string(), name);
    }
}

/// The loaded name bundle for `locale`, or `None` when no file shipped for it.
pub fn localized_names(locale: &str) -> Option<&'static LocalizedNames> {
    LOCALIZED_NAMES.get()?.get(locale)
}
...
/// SimulationCraft's own recommended consumables for a spec, keyed by the SimC
/// consumable keys (`flask`, `food`, `potion`, `augmentation`,
/// `temporary_enchant`). `class` and `spec` are the lowercase SimC tokens from
/// the profile's `<class>="..."` and `spec=` lines (`paladin`, `retribution`).
/// `None` when SimC ships no season profile for that spec.
pub fn recommended_consumables(
    class: &str,
    spec: &str,
) -> Option<&'static HashMap<String, String>> {
    SEASON_CONSUMABLES
        .get()?
        .get(&format!("{}/{}", class, spec))
}
...
mod tests {
    ...
    fn parses_localized_names_bundle_sections() {
        ...
        assert_eq!(
            names.instances.get(&1030).map(String::as_str),
            Some("塞塔里斯神庙")
        );
        assert_eq!(
            names.encounters.get(&2611).map(String::as_str),
            Some("首领")
        );
        assert_eq!(
            names.currencies.get(&3444).map(String::as_str),
            Some("勇士迷雾纹章")
        );
        assert_eq!(
            names.name_descriptions.get(&13567).map(String::as_str),
            Some("描述")
        );
        assert_eq!(names.maps.get(&2649).map(String::as_str), Some("地图"));
        assert_eq!(names.classes.get(&2).map(String::as_str), Some("圣骑士"));
        assert_eq!(names.specs.get(&70).map(String::as_str), Some("惩戒"));
        assert!(
            names.difficulties.is_empty(),
            "an omitted section must be empty, not fatal"
        );
    }

    // Guards the ITEM_NAMES merge: the bundle adds its locale to ids Raidbots
    // already ships, wins for that locale, leaves the other locales alone, and
    // brings in ids Raidbots has no entry for at all.
    #[test]
    fn localized_names_merge_wins_for_its_locale_only() {
        let mut base: HashMap<u64, HashMap<String, String>> = HashMap::from([(
            271465,
            HashMap::from([
                ("en_US".to_string(), "Warhelm".to_string()),
                ("zh_CN".to_string(), "stale".to_string()),
            ]),
        )]);
        merge_item_names(
            &mut base,
            "zh_CN",
            HashMap::from([
                (271465u64, "祝圣烈焰战盔".to_string()),
                (999u64, "新物品".to_string()),
            ]),
        );
        assert_eq!(base[&271465]["zh_CN"], "祝圣烈焰战盔");
        assert_eq!(base[&271465]["en_US"], "Warhelm");
        assert_eq!(base[&999]["zh_CN"], "新物品");
    }

    // Guards the data-dir scan: only `localized-names.<locale>.json` is a
    // bundle, so a backup or the compacted item-names file is never parsed as one.
    #[test]
    fn only_locale_suffixed_bundles_are_loaded() {
        assert_eq!(
            localized_names_locale(Path::new("/d/localized-names.zh_CN.json")),
            Some("zh_CN".to_string())
        );
        for other in [
            "/d/localized-names.json",
            "/d/localized-names.zh_CN.json.bak",
            "/d/localized-names.zhCN.json",
            "/d/item-names.json",
        ] {
            assert_eq!(localized_names_locale(Path::new(other)), None, "{other}");
        }
    }

    // Guards the 404 path of GET /api/localized-names/{locale}: a locale with
    // no bundle must miss rather than serve an empty one.
    #[test]
    fn localized_names_misses_for_a_locale_without_a_bundle() {
        assert!(localized_names("xx_XX").is_none());
    }
