...
pub(super) fn build_slot_candidates(
    ...
    for slot in GEAR_SLOTS {
        ...
        for item in slot_items {
            ...
            let mut keep = selected_uids.contains(&uid) || selected_identities.contains(&identity);
            // An upgrade variant rides along with whatever the user picked for
            // the item it was derived from — it has a uid of its own, so it
            // would otherwise be filtered out and the budget would buy nothing.
            if !keep {
                if let Some(base) = upgrade_variant_base_uid(&uid) {
                    keep = selected_uids.contains(base)
                        || selected_identities.contains(&uid_identity(base));
                }
            }
            if keep {
...
mod tests {
    ...
    fn upgrade_variants_survive_candidate_filtering() {
        ...
        let candidates = build_slot_candidates("", &items_by_slot, &selected);
