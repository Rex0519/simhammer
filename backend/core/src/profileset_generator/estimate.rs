...
fn gear_axis_size(
    ...
    for (slot, items) in items_by_slot {
        ...
        let variants = items
            ...
            .count() as u64;
        // Combined bound per slot:
        //   (selected + 1 equipped) + min(upgrade variants, that base), then ×2
        // if the slot has socket-added copies. At most one upgrade variant per
        // base candidate can be a candidate itself, and every surviving
        // candidate can carry at most one socketed twin.
        let mut axis = if variants == 0 && selected == 0 {
            1
        } else {
