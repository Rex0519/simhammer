...
finger2=,id=101\n";
    ...
    // ---- Top Gear enchant/gem combo-count edge cases ----
...
main_hand=,id=200\n";
    ...
    fn replace_gems_false_full_sockets_emits_nothing_via_iterator() {
        ...
        assert_eq!(
            exact, 0,
            "count should be 0 (no empty sockets, replace off)"
        );
        assert_eq!(
            iter_count, 0,
...
finger1=,id=400\nfinger2=,id=401\nmain_hand=,id=200\n"
        ...
        items.insert(
            "head".into(),
            vec![mk_simc("head", 100, ",id=100,enchant_id=7000", true, 0)],
        );
        items.insert("main_hand".into(), vec![mk("main_hand", 200, true, 0)]);
        items.insert(
            "finger1".into(),
            vec![mk("finger1", 400, true, 1), mk("finger1", 402, false, 1)],
        );
        items.insert(
            "finger2".into(),
            vec![mk("finger2", 401, true, 1), mk("finger2", 403, false, 1)],
        );
        let mut selected: HashMap<String, Vec<String>> = HashMap::new();
    ...
    fn insta_like_lock(
        ...
        assert_eq!(
            bundle, expected,
            "eager output drifted from golden snapshot"
        );
    }
    ...
    fn eager_emit_characterization_single_gear_swap() {
        ...
        let combo = metadata
            .get("Combo 2")
            .expect("Combo 2 metadata must exist");
        ...
        assert_eq!(
            off["is_kept"],
            json!(false),
            "off_hand is_kept must be false"
        );
        assert_eq!(
            off["origin"],
            json!("system"),
            "off_hand origin must be system"
        );
    }
    ...
    fn gem_apply_uses_per_item_socket_count_from_game_data() {
        ...
        for block in input_b.split("### ").skip(1) {
            if block.contains("neck=,id=250012") && block.contains("gem_id=") {
                panic!("Scenario B (eager): 0-socket equipped neck must NOT be gemmed:\n{block}");
            }
        }
    }
    ...
    fn iterator_metadata_matches_eager_per_combo() {
        ...
        for cand in iter {
            ...
            let expected = eager_body_to_meta.get(&key).unwrap_or_else(|| {
                panic!(
                    "eager has no metadata for simc body of {}",
                    cand.profileset_name
                )
            });
            ...
            assert_eq!(
                &actual, *expected,
                "metadata mismatch for {} (simc body lookup)",
                cand.profileset_name
            );
            checked += 1;
        }
        ...
        let eager_profilesets = eager_meta
            .keys()
            .filter(|k| k.starts_with("Combo "))
            .count();
        assert_eq!(
            checked, eager_profilesets,
            "iterator did not cover all eager profileset combos"
        );
    }
    ...
    fn returned_count_matches_emitted_blocks_and_count_fn() {
        ...
        assert_eq!(
            count, emitted,
            "returned count must match emitted ### Combo blocks"
        );
    ...
    #[test]
    fn locking_a_slot_drops_the_combo_count() {
        // regression for #146: locking a slot must remove it from the combination
        // space, so the count the UI shows falls without pressing Run.
        ensure_game_data_loaded();

        let base = "mage=test\nspec=frost\nhead=,id=250010\nwrist=,id=250011\n";
        let mut items_by_slot: HashMap<String, Vec<serde_json::Value>> = HashMap::new();
        let mut selected: HashMap<String, Vec<String>> = HashMap::new();
        for (slot, eq_id, alt_id) in [("head", 250010_u64, 301_u64), ("wrist", 250011, 302)] {
            let equipped = json!({
                "slot": slot, "simc_string": format!(",id={eq_id}"),
                "is_equipped": true, "origin": "equipped", "item_id": eq_id,
                "ilevel": 0, "name": "eq", "bonus_ids": [], "enchant_id": 0,
        ...
        let count = |locked: &HashSet<String>| {
            super::count_top_gear_combos_with_talents(
                base,
                &items_by_slot,
                &selected,
                None,
        ...
        let unlocked = count(&HashSet::new());
        let locked_head = count(&HashSet::from(["head".to_string()]));
        assert!(unlocked > 0, "baseline should emit combos");
        assert!(
            locked_head < unlocked,
            "locking head must shrink the space ({locked_head} vs {unlocked})"
        );
    }
