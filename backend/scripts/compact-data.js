...
const MANIFEST = {
  ...
  // localized-names.<locale>.json — minify only. One file per locale, so the
  // set is discovered at runtime (see LOCALIZED_NAMES_RE) rather than listed
  // here; generated by fetch-localized-names.mjs, absent until that has run.
};

/** Per-locale game-name bundles: manifest entries by pattern, copied minified. */
...
async function main() {
  ...
  for (const filename of manifestFiles) {
    ...
    // localized-names files are matched by pattern, not by key: `null` = minify.
    const config = filename in MANIFEST ? MANIFEST[filename] : null;
