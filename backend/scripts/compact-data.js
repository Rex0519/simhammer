...
const MANIFEST = {
  ...
  // Per-spec recommended consumables scraped from SimulationCraft's season
  // profiles — small file, keep as-is. Generated by
  // fetch-season-consumables.mjs, absent until that has run.
  "season-consumables.json": null,
  ...
  // Icon name → FileDataID for icons Blizzard's CDN won't serve by name.
  // Generated by fetch_icon_file_ids.py, so absent until that has run.
  "icon-file-ids.json": null,

  // localized-names.<locale>.json — `items` stripped to the same current-
  // expansion ids as item-names.json, every other table kept. One file per
  // locale, so the set is discovered at runtime (see LOCALIZED_NAMES_RE) rather
  // than listed here; generated by fetch-localized-names.mjs, absent until that
  // has run.
};

/** Per-locale game-name bundles: manifest entries by pattern (see main()). */
const LOCALIZED_NAMES_RE = /^localized-names\.[a-z]{2}_[A-Z]{2}\.json$/;
const LOCALIZED_NAMES_CONFIG = { custom: true, handler: "localizedNames" };
 ...
 */
function currentExpansionItemIds(inputDir) {
  const validIds = new Set();
  for (const filename of ["equippable-items-full.json", "encounter-items.json"]) {
...
function compactItemNames(inputPath, outputPath, inputDir) {
  ...
  const validIds = currentExpansionItemIds(inputDir);
...
/**
 * Compact localized-names.<locale>.json: strip `items` to the same current-
 * expansion ids compactItemNames keeps, copy every other table as-is.
 * Unfiltered the bundle is ~8 MB of item names per locale that the UI can never
 * ask for, shipped in every desktop build and served to every web user.
 */
function compactLocalizedNames(inputPath, outputPath, inputDir) {
  const data = JSON.parse(fs.readFileSync(inputPath, "utf8"));
  if (!data.items) {
    fs.writeFileSync(outputPath, JSON.stringify(data));
    return;
  ...
  for (const [id, name] of Object.entries(data.items)) {
    if (validIds.has(id)) filtered[id] = name;
  }
  data.items = filtered;

  fs.writeFileSync(outputPath, JSON.stringify(data));
  console.log(`    (kept ${Object.keys(filtered).length}/${total} localized item names)`);
}
...
async function compactFile(inputPath, outputPath, config, inputDir, outputDir) {
  if (config && config.custom) {
    ...
    } else if (config.handler === "localizedNames") {
      compactLocalizedNames(inputPath, outputPath, inputDir);
    } else {
...
async function main() {
  ...
  // Process only JSON files listed in the MANIFEST, plus every per-locale
  // localized-names bundle present in the input dir.
  const manifestFiles = [
    ...Object.keys(MANIFEST),
    ...fs.readdirSync(inputDir).filter((f) => LOCALIZED_NAMES_RE.test(f)),
  ];
  ...
  for (const filename of manifestFiles) {
    ...
    // localized-names files are matched by pattern, not by key.
    const config = filename in MANIFEST ? MANIFEST[filename] : LOCALIZED_NAMES_CONFIG;
